...
class UserLicenseSettingsServiceTest {
    ...
    // ===== User Limit Tests =====
    // The license-based user limit has been removed: calculateMaxAllowedUsers() reports an
    // unlimited number of seats regardless of license type or grandfathering.

    @Test
    void noLicense_returnsUnlimited() {
        // No license active
        ...
        assertEquals(
                Integer.MAX_VALUE,
                result,
                "User limit removed: unlimited users allowed without a license");
    }
    ...
    @Test
    void enterpriseLicense_returnsUnlimited() {
        // ENTERPRISE license seat count no longer caps the number of users
        when(premium.isEnabled()).thenReturn(true);
        ...
        assertEquals(
                Integer.MAX_VALUE,
                result,
                "User limit removed: ENTERPRISE seat count no longer caps users");
    }

    @Test
    void grandfatheringIgnored_returnsUnlimited() {
        // Grandfathered count no longer affects the number of allowed users
        mockSettings.setGrandfatheredUserCount(80);
        when(premium.isEnabled()).thenReturn(false);
        ...
        assertEquals(
                Integer.MAX_VALUE,
                result,
                "User limit removed: grandfathered count no longer caps users");
    }
